...
exports.indexControllers = {
    getClients: async (req, res) => {
        try {
            const [result] = await app_1.connectionSQL.pool.query('SELECT * FROM info_paciente ORDER BY create_at DESC');
            res.json(result);
        }
        catch (error) {
            res.json(error);
    ...
    getClient: async (req, res) => {
        ...
        try {
            ...
            const [count] = await app_1.connectionSQL.pool.query('SELECT create_at FROM _antropometria WHERE id_paciente = ? ORDER BY create_at DESC', [params]);
            if (count.length === 0) {
                return res.json({
                    //"paciente":result[0],
                    //"query":{
            ...
            const [result3] = await app_1.connectionSQL.pool.query('SELECT * FROM _bioimpedanciometria WHERE id_paciente = ? AND YEAR(create_at) = ? AND MONTH(create_at) = ? AND DAY(create_at) = ? LIMIT 1', [params, year, month, day]);
            res.json({
                //"paciente":result[0],
                //"query":{
                "id_paciente": params,
    ...
    getClientExamenes: async (req, res) => {
        ...
        try {
            //const [result]:any = await connectionSQL.pool.query('SELECT * FROM info_paciente WHERE id_paciente = ? LIMIT 1', [params])
            const [count] = await app_1.connectionSQL.pool.query('SELECT create_at FROM _examenes_laboratorios WHERE id_paciente = ? ORDER BY create_at DESC', [params]);
            if (count.length === 0) {
                return res.json({
                    //"paciente":result[0],
                    //"query":{
